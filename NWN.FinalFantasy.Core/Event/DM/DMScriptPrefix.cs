namespace NWN.FinalFantasy.Core.Event.DM
{
    internal class DMScriptPrefix
    {
        public const string OnAppear = "DM_ON_APPEAR_";
        public const string OnChangeDifficulty = "DM_ON_CHANGE_DIFFICULTY_";
        public const string OnDisableTrap = "DM_ON_DISABLE_TRAP_";
        public const string OnDisappear = "DM_ON_DISAPPEAR_";
        public const string OnForceRest = "DM_ON_FORCE_REST_";
        public const string OnGetVariable = "DM_ON_GET_VARIABLE_";
        public const string OnGiveGold = "DM_ON_GIVE_GOLD_";
        public const string OnGiveItem = "DM_ON_GIVE_ITEM_";
        public const string OnGiveLevel = "DM_ON_GIVE_LEVEL_";
        public const string OnGiveXP = "DM_ON_GIVE_XP_";
        public const string OnHeal = "DM_ON_HEAL_";
        public const string OnJump = "DM_ON_JUMP_";
        public const string OnJumpAll = "DM_ON_JUMP_ALL_";
        public const string OnJumpTarget = "DM_ON_JUMP_TARGET_";
        public const string OnKill = "DM_ON_KILL_";
        public const string OnLimbo = "DM_ON_LIMBO_";
        public const string OnPossess = "DM_ON_POSSESS_";
        public const string OnSetDate = "DM_ON_SET_DATE_";
        public const string OnSetStat = "DM_ON_SET_STAT_";
        public const string OnSetTime = "DM_ON_SET_TIME_";
        public const string OnSetVariable = "DM_ON_SET_VARIABLE_";
        public const string OnSpawnCreature = "DM_ON_SPAWN_CREATURE_";
        public const string OnSpawnEncounter = "DM_ON_SPAWN_ENCOUNTER_";
        public const string OnSpawnItem = "DM_ON_SPAWN_ITEM_";
        public const string OnSpawnPlaceable = "DM_ON_SPAWN_PLACEABLE_";
        public const string OnSpawnPortal = "DM_ON_SPAWN_PORTAL_";
        public const string OnSpawnTrap = "DM_ON_SPAWN_TRAP_";
        public const string OnSpawnTrigger = "DM_ON_SPAWN_TRIGGER_";
        public const string OnSpawnWaypoint = "DM_ON_SPAWN_WAYPOINT_";
        public const string OnTakeItem = "DM_ON_TAKE_ITEM_";
        public const string OnToggleImmortality = "DM_ON_TOGGLE_IMMORTALITY_";
        public const string OnToggleAI = "DM_ON_TOGGLE_AI_";
        public const string OnToggleLock = "DM_ON_TOGGLE_LOCK_";
    }
}
